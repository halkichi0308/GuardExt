...
// If you want to use a custom block list, change this URL.
const REMOTE_BLOCKLIST_URL =
    "https://halkichi0308.github.io/GuardExt/data/malicious_ids.json";
...
async function loadBlocklist() {
    if (REMOTE_BLOCKLIST_URL) {
        try {
            const resp = await fetch(REMOTE_BLOCKLIST_URL, {
                cache: "no-cache",
            });
            if (resp.ok) {
            ...
            console.warn(
                "GuardExt: remote blocklist fetch failed, using local copy",
                e,
            );
        }
    }
